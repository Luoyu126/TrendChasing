...
当前使用 `ghcr.io/rachelos/we-mp-rss:latest`，服务地址为 `http://127.0.0.1:8001`，数据库及数据持久化到 `output/werss/`。打开管理页面、登录、扫码授权、添加公众号并确认文章更新，再复制服务实际生成的 RSS 订阅地址。
...
将 RSS 地址填入下一步的 `wechat.feeds`。此方式由采集器直接读取 WeRSS。代码还保留 RSSHub 的 `/wechat/sogou/:id` 入口，对应 `wechat.accounts`；本机此前测试仅得到旧文章摘要等不完整结果，因此公众号日常接入使用 WeRSS。
...
| 微信公众号 | `wechat.feeds` 中的 WeRSS URL | 已授权并订阅的公众号文章 |
...
| 路径 | 内容 |
| --- | --- |
| `output/content_pool/content.sqlite3` | 候选、正式池、处理缓存、日报批次与投递记录 |
| `output/content_pool/reports/` | 尚未清理的 HTML / JSON 日报 |
| `output/papers/state.sqlite3` | 论文元数据与评分缓存，独立保留 |
